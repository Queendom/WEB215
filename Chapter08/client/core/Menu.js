...
const isPartActive = (history, path) => {
  if (history.location.pathname.includes(path))
    return {color: '#faffed', backgroundColor: '#c3ff00', marginRight:10}
  else
    return {color: '#616161', backgroundColor: '#faffed', border:'1px solid #b7ff00', marginRight:10}
}
const Menu = withRouter(({history}) => (
  <AppBar position="static">
    <Toolbar>
      ...
      <Link to="/shops/all">
        <Button style={isActive(history, "/users")}>All Shops</Button>
      </Link>
